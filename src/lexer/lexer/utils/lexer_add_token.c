...
/*   Updated: 2021/10/31 11:03:11 by bgenia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
)
{
	...
	if (type == TOKEN_WORD)
	{
		...
		if (is_quote(lexer->source[position]))
		{
			token.expandable = lexer->source[position] == '"';
			token.position += 1;
			token.length -= 1;
			if (is_quote(lexer->source[position + token.length + 1]))
				token.length -= 1;
		}
	}
